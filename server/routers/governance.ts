...
export const governanceRouter = router({
  ...
  roles: router({
    list: adminProcedure.query(async () => (await database()).select().from(accessRoles).orderBy(asc(accessRoles.label))),
    ...
    permissions: adminProcedure.input(z.object({ accessRoleId: z.number().int().positive() })).query(async ({ input }) => (await database()).select().from(rolePermissions).where(eq(rolePermissions.accessRoleId, input.accessRoleId))),
  ...
  permissions: router({
    user: adminProcedure.input(z.object({ userId: z.number().int().positive() })).query(async ({ input }) => {
      const db = await database();
  ...
  audit: router({
    list: adminProcedure.input(z.object({ module: z.string().trim().max(80).optional(), limit: z.number().int().min(1).max(250).default(100) }).optional()).query(async ({ input }) => {
      const db = await database();
