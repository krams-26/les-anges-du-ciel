...
import { z } from "zod";
import { attendanceRecords, attendanceSessions, grades, teacherReports, teachers, teachingAssignments } from "../../drizzle/schema";
...
async function assertAssignmentAccess(userId: number, userRole: "user" | "admin", assignmentId: number) {
  ...
  const matches = await db.select({ id: teachingAssignments.id }).from(teachingAssignments).innerJoin(teachers, eq(teachingAssignments.teacherId, teachers.id)).where(and(eq(teachingAssignments.id, assignmentId), eq(teachers.userId, userId))).limit(1);
  if (!matches.length) throw new TRPCError({ code: "FORBIDDEN", message: "Cette affectation ne vous autorise pas à effectuer cette opération." });
}

const attendanceInput = z.object({ assignmentId: z.number().int().positive(), sessionDate: z.coerce.date(), records: z.array(z.object({ enrollmentId: z.number().int().positive(), status: z.enum(["present", "absent", "late", "excused"]), note: z.string().trim().max(500).optional() })).min(1) });
const gradesInput = z.object({ assignmentId: z.number().int().positive(), periodId: z.number().int().positive(), scores: z.array(z.object({ enrollmentId: z.number().int().positive(), score: z.number().int().min(0), maximum: z.number().int().min(1).max(100) })).min(1) }).refine((value) => value.scores.every((score) => score.score <= score.maximum), { message: "Une note ne peut pas dépasser son maximum." });

export const teachingRouter = router({
  myAssignments: protectedProcedure.query(async ({ ctx }) => {
    const db = await dbOrThrow();
    if (ctx.user.role === "admin") return db.select().from(teachingAssignments);
    const [teacher] = await db.select({ id: teachers.id }).from(teachers).where(eq(teachers.userId, ctx.user.id)).limit(1);
    if (!teacher) return [];
    return db.select().from(teachingAssignments).where(eq(teachingAssignments.teacherId, teacher.id));
  }),
  attendance: router({
    save: protectedProcedure.input(attendanceInput).mutation(async ({ ctx, input }) => {
  ...
  grades: router({
    saveDraft: protectedProcedure.input(gradesInput).mutation(async ({ ctx, input }) => {
      await assertAssignmentAccess(ctx.user.id, ctx.user.role, input.assignmentId); const db = await dbOrThrow();
    ...
    submit: protectedProcedure.input(gradesInput).mutation(async ({ ctx, input }) => {
      await assertAssignmentAccess(ctx.user.id, ctx.user.role, input.assignmentId); const db = await dbOrThrow();
