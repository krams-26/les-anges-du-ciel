...
export function StudentCsvImportModal({ open, onClose, onImport, onSuccess }: { open: boolean; onClose: () => void; onImport: (rows: CsvStudentRow[]) => void; onSuccess: (title: string, description: string) => void }) {
  ...
  const confirmImport = () => {
    ...
    setIsImporting(true);
    window.setTimeout(() => {
      onImport(validRows.map(({ errors, ...row }) => row));
